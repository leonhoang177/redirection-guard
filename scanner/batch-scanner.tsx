...
const INPUT_FILE = "./inputs/input.txt";
...
function readUrlList(path: string): string[] {
  try {
    ...
    console.error(`Could not read ${path}:`, (e as any)?.message || String(e));
    return [];
  }
}
...
async function run() {
  // If a single URL is passed as arg, process just that; otherwise read input.txt
  const arg = process.argv[2];
  const singleUrl = arg && /^(https?:)?\/\//i.test(arg) ? arg : undefined;
  const urls = singleUrl ? [singleUrl] : readUrlList(INPUT_FILE);

  if (!urls.length) {
    console.error(
      `No URLs to process. Add them to ${INPUT_FILE} (one per line), or pass a URL as an argument.`
    );
    process.exit(1);
  }

  ensureOutputsDir();
  const outPath = "./outputs/output.ndjson"; // NDJSON: one JSON object per line
  const waitlistPath = "./outputs/waitlist.txt";
  ...
  for (const target of urls) {
    ...
    try {
      ...
      try {
        await submitUrl(target); // start analysis, but do not poll
      } catch (_) {}
      try {
        fs.appendFileSync(waitlistPath, `${target}\n`, { encoding: "utf-8" });
      } catch (_) {}
    ...
    try {
      ...
      appendJsonLine(outPath, output);
    } catch (e: any) {
      console.error(
        `Failed to build metadata for ${target}:`,
        e?.message || String(e)
      );
    }
  }

  console.log(`Done. Appended results to ${outPath}`);
}
